...
fn main() {
    let cmd = Command::new("tpw")
        ...
        .subcommand(Command::new("md2obj").arg(Arg::new("file").required(true)))
        .subcommand(Command::new("map").arg(Arg::new("file").required(true)))
    ...
    } else     if let Some(("map", m)) = cmd.subcommand() {
